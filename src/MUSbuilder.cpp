...
//for dl2asp -- ychen
#include "dlsolver.h"
extern vector<dlint_set> fmcs;
extern vector<dlint_set> fmus;
...
bool MUSbuilder::constructMUS ( list<Cover> covers, ClauseAssign curAssign )
{
    ...
    if ( verbose ) {
        printf ( "<--constructMUS exhausted options\n" );
    }
...
// remove all occurrences of a clause and covers in which it appears,
// as well as all clauses in a particular cover
...
// Check to see if we've visited this assignment before
...
inline void MUSbuilder::outputMUS ( ClauseAssign& curAssign )
{
    //for dl2asp -- ychen
    dlint_set mus;
    for ( vector<Num>::iterator it = singletons.begin(); it != singletons.end(); it++ ) {
        mus.insert ( *it );
    }

    for ( Num i = 0 ; i < curAssign.size() ; i++ ) {
        if ( curAssign[i] == 1 ) {
            mus.insert ( mapToOrig ( i ) );
        }
    }
    fmus.push_back ( mus );
...
//  - Provides a lower bound on the number of elements needed to hit the
//    remaining MCSes (covers)
